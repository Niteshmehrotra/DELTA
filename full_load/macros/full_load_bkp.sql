...
{% macro full_load_bkp(database_name,schema_name,table_name,SOURCE) %}
    {% set database_name = database_name|lower %}
    {% set schema_name = schema_name|lower %}
    {% set table_name = table_name|lower %}

    {% if execute %}
        {% set DIFFHASH=hash_columns(database_name, schema_name, table_name) %}

        {% set NKHASH=natural_columns(database_name, schema_name, table_name) %}


        {% set COLS=column_list(database_name, schema_name, table_name) %}
...
{% endif %}
	WITH BATCH_LIST as (
		...
		from
		    --"LANDING"."DEV_SAL"."CONTACT_STREAM"
			{{SOURCE}}
	...
	, BATCH_DATA as (
		...
		from
		    --"LANDING"."DEV_SAL"."CONTACT_STREAM" SRC
			{{SOURCE}} SRC
